...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const fetchUserWithCache = async (userId: string) => {
    ...
    try {
      ...
      // Adicionar timeout para evitar travamento (reduzido para melhor UX)
  ...
  useEffect(() => {
    const checkUser = async () => {
      try {
        ...
        if (session?.user) {
          // Sessão encontrada, definir usuário básico imediatamente para melhor UX
          const basicUser = {
            id: session.user.id,
          ...
          // Tentar buscar dados completos em background
          try {
