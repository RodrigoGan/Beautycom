...
const ConfiguracoesAgenda = () => {
  ...
  const { hasActiveAgenda, agendaType, loading: agendaStatusLoading } = useAgendaStatus(user?.id)
  
  // ✅ LÓGICA SIMPLIFICADA: Qualquer profissional com agenda ativa pode configurar
  // Removido: dependência de salão - usar apenas lógica de assinatura ativa
  
  // Qualquer profissional com agenda ativa pode gerenciar suas próprias configurações
  const canManageSettings = user?.user_type === 'profissional' && hasActiveAgenda
  
  console.log('🔍 ConfiguracoesAgenda - Verificação de permissões:', {
    userId: user?.id,
  ...
  // ✅ CORRIGIDO: Hook para configurações de agenda
  // Usar apenas lógica de assinatura ativa (independente de salão)
  const agendaSalonId = undefined
  ...
  } = useAgendaConfig(user?.id, agendaSalonId)
  
  console.log('🔍 useAgendaConfig - Parâmetros:', {
