...
const NovoAgendamento = () => {
  ...
  // Removido: dependência de userSalon - usar apenas lógica de assinatura ativa
  console.log('NovoAgendamento - User:', user?.id)
  ...
  useEffect(() => {
    ...
    const fetchServicos = async () => {
      try {
        ...
        // Buscar todos os serviços ativos do profissional (independente de salão)
        // A lógica de permissão é baseada apenas em assinatura ativa
  ...
  // Carregar profissionais disponíveis (baseado em assinatura ativa)
  useEffect(() => {
    const fetchProfissionais = async () => {
      try {
        ...
        // Buscar todos os profissionais com agenda ativa (independente de salão)
        ...
        console.log('NovoAgendamento - Profissionais com agenda ativa encontrados:', profissionaisData?.length || 0)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Buscar cliente pelo nome digitado se não foi selecionado do dropdown
    let clienteFinalTemp = clienteSelecionado
    if (!clienteFinalTemp && formData.cliente.trim()) {
      // Primeiro tentar busca exata
      clienteFinalTemp = clientes.find(cliente => 
        cliente.name.toLowerCase() === formData.cliente.toLowerCase()
      )
      
      // Se não encontrar, tentar busca parcial
      if (!clienteFinalTemp) {
        clienteFinalTemp = clientes.find(cliente => 
          cliente.name.toLowerCase().includes(formData.cliente.toLowerCase())
        )
      }
    }
    ...
    // Definir o cliente final para o modal
...
const confirmAppointment = async () => {
  ...
  try {
    ...
    const appointmentData = {
      salon_id: null, // Removido: dependência de salão - usar apenas lógica de assinatura ativa
  ...
  // Verificar se o usuário tem agenda ativa (baseado em assinatura)
  console.log('NovoAgendamento - Verificando agenda ativa:', { 
    userId: user?.id,
    userType: user?.user_type,
    agendaEnabled: (user as any)?.agenda_enabled
  })
  
  // Verificar se o usuário pode criar agendamentos (baseado em assinatura ativa)
  const canCreateAppointments = () => {
    // Verificar se tem agenda ativa (trial, assinatura ou habilitada)
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="relative">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-10"
                    />
                    {(clienteSelecionado || clienteFinal) && (
              ...
              {formData.data && formData.servico && formData.profissional ? (
                <TimeSlotSelector
                  ...
                  salonId={''} // Removido: dependência de salão
