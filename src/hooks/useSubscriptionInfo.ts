...
export const useSubscriptionInfo = (userId?: string) => {
  ...
  const fetchSubscriptionInfo = useCallback(async () => {
    ...
    try {
      ...
      console.log('🔍 Buscando trial para professional_id:', targetUserId)
      
      // Primeiro, buscar trial ativo não convertido
      ...
      console.log('🔍 Resultado da busca de trial ativo não convertido:', { trialData, trialError })
      
      // Se não encontrou trial não convertido, buscar qualquer trial ativo
      if (trialError && trialError.code === 'PGRST116') {
        console.log('🔍 Trial não convertido não encontrado, buscando qualquer trial ativo...')
        
        const { data: anyTrialData, error: anyTrialError } = await supabase
          .from('professional_trials')
          .select('*')
          .eq('professional_id', targetUserId)
          .eq('status', 'active')
        ...
        console.log('🔍 Resultado da busca de qualquer trial ativo:', { anyTrialData, anyTrialError })
        
        if (anyTrialData) {
          console.log('✅ Trial encontrado (pode estar convertido):', anyTrialData)
          // Se encontrou trial convertido, tratar como assinatura
          if (anyTrialData.converted_to_paid) {
            console.log('🔄 Trial foi convertido para assinatura paga')
            // Buscar a assinatura correspondente
            const { data: convertedSubscription } = await supabase
              .from('user_subscriptions')
              .select(`
                *,
                plan:subscription_plans(
            ...
            if (convertedSubscription) {
              console.log('✅ Assinatura convertida encontrada:', convertedSubscription)
              const periodEndDate = new Date(convertedSubscription.current_period_end)
              const now = new Date()
              ...
              setSubscriptionSummary({
                type: 'subscription',
                status: convertedSubscription.status,
                planName: convertedSubscription.plan.name,
                maxProfessionals: convertedSubscription.plan.max_professionals,
                currentProfessionals: 1, // Trial convertido = 1 profissional
                expirationDate: convertedSubscription.current_period_end,
                daysRemaining,
                isActive: convertedSubscription.status === 'active',
                subscriptionInfo: convertedSubscription
              })
              return
            }
          }
      ...
      if (trialData) {
        console.log('✅ Trial ativo não convertido encontrado:', trialData)
        ...
        console.log('🔍 Trial Debug:', {
          trialEndDate: trialData.end_date,
      ...
      // 3. Se não tem trial nem assinatura, verificar se há trial expirado
      console.log('🔍 Nenhum trial ativo ou assinatura encontrada, verificando trials expirados...')
      ...
      if (expiredTrialData) {
        console.log('🔍 Trial expirado encontrado:', expiredTrialData)
        const trialEndDate = new Date(expiredTrialData.end_date)
        const now = new Date()
        const timeDiff = trialEndDate.getTime() - now.getTime()
        const daysRemaining = Math.max(0, Math.floor(timeDiff / (1000 * 60 * 60 * 24)))
      ...
      // 4. Se não tem trial nem assinatura
      setSubscriptionSummary({
        type: 'none',
        status: 'expired',
        isActive: false
