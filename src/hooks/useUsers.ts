...
export function useUsers(filters: UserFilters = {}, itemsPerPage: number = 20) {
  ...
  const fetchUsers = useCallback(async (filters: UserFilters = {}, resetPage = false) => {
    ...
    try {
      ...
      let usersQuery = supabase
        ...
        .range(0, itemsPerPage - 1) // Usar parâmetro configurável
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      // ✅ CORREÇÃO: Usar length atual em vez de currentOffset
      let usersQuery = supabase
        .from('users')
        .select('*')
        .order('created_at', { ascending: false })
        .range(users.length, users.length + itemsPerPage - 1)
      ...
      if (usersData && usersData.length > 0) {
        ...
        hasMoreUsers = newUsers.length === itemsPerPage
      } else {
        hasMoreUsers = false
      }

      // Carregar salões se necessário
      if (filters.userType === 'profissional' || !filters.userType) {
        let salonsQuery = supabase
          ...
          `)
          .order('created_at', { ascending: false })
          .range(salons.length, salons.length + itemsPerPage - 1)
        ...
        if (salonsData && salonsData.length > 0) {
          ...
          hasMoreSalons = newSalons.length === itemsPerPage
        } else {
          hasMoreSalons = false
        }
      }

      // ✅ CORREÇÃO: Considerar ambos os tipos para hasMore
      setHasMore(hasMoreUsers || hasMoreSalons)
  ...
  }, [loading, hasMore, users.length, salons.length, filters, itemsPerPage])
