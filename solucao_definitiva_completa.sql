-- SOLUÇÃO DEFINITIVA COMPLETA: Desabilitar RLS em TODAS as tabelas
-- Este script resolve TODOS os problemas de políticas RLS

-- 1. PRIMEIRO: Desabilitar RLS em TODAS as tabelas relacionadas
...
-- Políticas da tabela salons_studios
DROP POLICY IF EXISTS "Funcionários ver salão" ON salons_studios;
DROP POLICY IF EXISTS "Dono pode gerenciar salão" ON salons_studios;
DROP POLICY IF EXISTS "Usuários podem ver salões" ON salons_studios;
DROP POLICY IF EXISTS "Salon owners can manage their salon" ON salons_studios;
DROP POLICY IF EXISTS "Users can view all salons" ON salons_studios;
DROP POLICY IF EXISTS "Funcionários podem ver salão" ON salons_studios;

-- Políticas da tabela salon_professionals
...
-- Políticas da tabela salon_employees
...
DROP POLICY IF EXISTS "Funcionário pode ver próprios dados" ON salon_employees;
DROP POLICY IF EXISTS "Funcionário pode aceitar/rejeitar convites" ON salon_employees;
...
SELECT 
    ...
    rowsecurity
FROM pg_tables 
WHERE tablename IN ('salons_studios', 'salon_employees', 'salon_professionals', 'salon_invitations');

-- 16. Verificar se há políticas RLS restantes
SELECT 
