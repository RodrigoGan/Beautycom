-- Análise detalhada do post problemático e comparação com outros
-- Post ID: 2d1e6121-5485-4633-8fa1-2e84c68d631a (que aparece na imagem)
...
-- 3. VERIFICAR SE HÁ POSTS COM PRIORIDADES DUPLICADAS
SELECT 
    'PRIORIDADES DUPLICADAS' as analise,
    salon_main_post_priority,
    COUNT(*) as quantidade,
    string_agg(p.id, ', ') as post_ids,
    string_agg(p.title, ', ') as titulos
FROM posts p
WHERE p.is_salon_main_post = true
AND p.salon_main_post_priority IS NOT NULL
...
-- 7. VERIFICAR SE O POST PROBLEMÁTICO TEM ALGUM TRIGGER OU CONSTRAINT ESPECIAL
SELECT 
